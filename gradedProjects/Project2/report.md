...
$P(either = value \mid xray = yes) ∝  Σ_{smoke} Σ_{asia} Σ_{lung} Σ_{tub}$
   ...
   1. Using the factorization of the Asia network:
   $P(either, xray, smoke, asia, lung, tub)= P(asia) \cdot P(tub \mid asia) \cdot P(smoke) \cdot P(lung \mid smoke)\cdot P(either \mid lung, tub) \cdot P(xray \mid either)$
    Note: We omit bronc and dysp from the factorization since their factors sum to 1 marginalized and do not change the result.
2. Compute required prior probabilities
   1. _P(lung = yes) and P(lung = no)_:
  ...
  3. _P(either = yes) and P(either = no)_:
...
3. Include the xray evidence
From the CPT:
$P(xray = yes \mid either = yes) = 0.98$
$P(xray = yes \mid either = no) = 0.05$
   1. Compute unnormalized values:
      1. $P(either = yes, xray = yes)$
         $= 0.064828 \cdot 0.98 = 0.06353144$
      2. $P(either = no, xray = yes)$
...
and therefore $P(\text{either} = \text{yes} \mid \text{xray} = \text{yes}, \text{tub} = \text{yes}) = 1.$
...
Verified using a sampling method (likelihood weighting):
because the relation is deterministic, even 1 sample is enough and gives the same answer as, for example, 5000 samples.
