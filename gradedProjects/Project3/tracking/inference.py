...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        """
        total = self.total()
        r = random.random() * total   # random point in [0, total)
        cumulative = 0.0
        ...
        # Fallback: return last key (in case of floating-point edge cases)
        return key
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if ghostPosition == jailPosition:
            # Only consistent observation is None
            if noisyDistance is None:
                return 1.0
            else:
                return 0.0

        # Case 2: ghost is not in jail but we observed "no ghost" (None)
        if noisyDistance is None:
            return 0.0

        # Case 3: normal case – use manhattan distance and busters model
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        # For every old position, spread its probability mass over successor positions
        for oldPos in self.allPositions:
            oldProb = self.beliefs[oldPos]
            if oldProb == 0:
                continue  # skip useless work

            # Distribution over new positions given the old position
            newPosDist = self.getPositionDistribution(gameState, oldPos)

            for newPos, prob in newPosDist.items():
                newBeliefs[newPos] += oldProb * prob

        # Replace old beliefs with the predicted ones
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        # Evenly distribute particles over legal positions
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        # 1. Build a weight distribution over positions
        weights = DiscreteDistribution()
        for p in self.particles:
            prob = self.getObservationProb(observation, pacmanPosition, p, jailPosition)
            weights[p] += prob

        # 2. Handle special case: all weights are zero
        if weights.total() == 0:
            self.initializeUniformly(gameState)
            return

        # 3. Resample particles from the weight distribution
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            # Cache the transition distribution for each distinct old position
            if oldPos not in distCache:
                distCache[oldPos] = self.getPositionDistribution(gameState, oldPos)

            newPosDist = distCache[oldPos]
